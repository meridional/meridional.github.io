...
function start_gl(canvas_id, vertexShader, fragmentShader) {
   setTimeout(function() {
      ...
      function setMouse(event, eventZ) {
         ...
         gl.dest.x = gl.cursor.x + ((event.clientX - r.left) / (r.right - r.left) * 2 - 1)*gl.scale;
         gl.dest.y = gl.cursor.y + ((event.clientY - r.bottom) / (r.top - r.bottom) * 2 - 1)*gl.scale;
         gl.destScale *= eventZ;
         //if (gl.destScale < 1e-5) gl.destScale = gl.destScale;
         //else gl.destScale *= eventZ;
      }
      gl.dest = new Vector3();
  ...
  //    canvas.onmousedown = function(event) { setMouse(event, 1); } // On mouse press, set z to 1.
      //canvas.ondblclick = function(event) {setMouse(event,4.);}
      canvas.onclick = function(event) {setMouse(event,.7);}
      //canvas.onmousemove = function(event) { mouseMove(event, 1) ; }
...
function gl_init(gl, vertexShader, fragmentShader) {
   ...
   ['uTime','uCursor', 'uScale'].forEach(function(name) {
...
function gl_update(gl) {
   ...
   gl.uniform1f(gl.uScale, gl.scale);
   gl.scale *= Math.pow(gl.destScale / gl.scale, 0.05);
   gl.cursor.x += (gl.dest.x - gl.cursor.x) * Math.sqrt(gl.scale)*0.25;
   gl.cursor.y += (gl.dest.y - gl.cursor.y) * Math.sqrt(gl.scale)*0.25;
