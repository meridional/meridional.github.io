...
class ThreeJSTest {
  ...
  constructor(rows : number, cols : number) {
    //return;
    function indexToCoordinate(i, j) : THREE.Vector3 {
...
function start(test : ThreeJSTest) {
  test.turn();
  test.renderer.render(test.scene, test.camera);
  kd.tick();
  requestAnimationFrame(() => {start(test)});
}

window.onload = () => {
  console.log("hello");
  var three = new ThreeJSTest(20, 20);
  kd.W.down(function() {three.moveForward();});
  kd.A.down(function() {three.turnLeft();});
  kd.D.down(function() {three.turnRight();});
  kd.S.down(function() {three.moveBackward();});
  start(three);
};